...
Gtk::Button *ExampleWindow::create_color_swatch(int swatch_i) {
  ...
  drawing_area->set_draw_func(sigc::bind(
      sigc::mem_fun(*this, &ExampleWindow::on_drawing_area_draw), swatch_i));

  return color_swatch;
}

void ExampleWindow::on_drawing_area_draw(
    const Cairo::RefPtr<Cairo::Context> &cr, int, int, int swatch_i) {
